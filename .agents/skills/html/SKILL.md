...
For a **password-protected** site this endpoint is owner-only — add
`-H "Authorization: Bearer {update_key}"`, or it returns `404` (a protected
site's details are never exposed without the owner key).
