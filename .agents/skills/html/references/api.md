...
Get a site's status and the assets referenced by its HTML. Public sites need no
auth. A **password-protected** site is owner-only here: send
`Authorization: Bearer {update_key}`. Without the owner key it returns `404` and
never exposes the protected site's details.
...
**Not found (404):** no such site — or the site is password-protected and you
didn't send the owner's `update_key`.
