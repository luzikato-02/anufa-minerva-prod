...
class StockSheetSessionScreen extends ConsumerStatefulWidget {
  const StockSheetSessionScreen({super.key});

  @override
  ConsumerState<StockSheetSessionScreen> createState() => _StockSheetSessionScreenState();
}

class _StockSheetSessionScreenState extends ConsumerState<StockSheetSessionScreen> {
  ...
  Future<void> _startNew() async {
    final current = ref.read(stockSheetProvider);
    if (current != null && current.rows.isNotEmpty) {
      final ok = await confirmDialog(context, title: 'Start a new sheet?', message: 'The ${current.rows.length} rows on the current sheet are already saved. You will begin an empty sheet.', confirmLabel: 'Start new sheet');
      if (!ok || !mounted) return;
    }
    await ref.read(stockSheetProvider.notifier).startNew();
    if (mounted) context.go('/stock-sheet');
  }
  ...
  Future<void> _loadSession() async {
    ...
    try {
      await ref.read(stockSheetProvider.notifier).loadSession(id);
      if (mounted) context.go('/stock-sheet');
    ...
    return MinervaScaffold(
      ...
      body: ListView(padding: const EdgeInsets.all(16), children: [
        AppCard(
          ...
          description: 'Begin a fresh stock sheet.',
